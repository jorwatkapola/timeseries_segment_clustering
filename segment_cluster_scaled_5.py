...
def reconstruct(test_segments, test_ts, kmeans_model, rel_offset=True, seg_slide=25):
    ...
    error=0
    centroids=np.copy(kmeans_model.cluster_centers_)
    if np.shape(test_segments)[1] == 2:
        ...
        for n_seg, segment in enumerate(test_segments):
            std_segment=np.std(np.array(segment))
            mean_segment=np.mean(np.array(segment))

            kmeans_model.cluster_centers_=np.mean(centroids)+(kmeans_model.cluster_centers_-np.mean(kmeans_model.cluster_centers_))*(std_segment/np.std(kmeans_model.cluster_centers_))
            
            
            pred_centroid_index=kmeans_model.predict(np.array(segment).reshape(1, -1))[0]
            pred_centroid=kmeans_model.cluster_centers_[pred_centroid_index]
