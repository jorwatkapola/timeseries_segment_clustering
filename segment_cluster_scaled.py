...
def reconstruct(test_segments, test_ts, kmeans_model, rel_offset=True, seg_slide=25):
    ...
    if np.shape(test_segments)[1] == 2:
        ...
        for n_seg, segment in enumerate(test_segments):
            ...
            kmeans_model.cluster_centers_=mean_segment+(kmeans_model.cluster_centers_-np.mean(kmeans_model.cluster_centers_))*(std_segment/np.std(kmeans_model.cluster_centers_))
